...
public class OsmosisSerializer extends BinarySerializer implements Sink {
	...
	class NodeGroup extends Prim<Node> implements PrimGroupWriterInterface {
		...
		public void serialize(Osmformat.PrimitiveBlock.Builder parentbuilder) {
			...
			for (Node i : contents) {
				...
				if (i.getTags().size() > 0) {
					// Not a dense node. Do we have any dense nodes that we need to flush,
					...
					if (densenodes.size() > MIN_DENSE) {
						...
						// Nope. Just queue the dense nodes as if they weren't dense.
		...
		public void serializeNonDense(Osmformat.PrimitiveBlock.Builder parentbuilder,
				List<Node> nodes) {
			if (nodes.size() == 0)
				return;
			...
			for (Node i : nodes) {
				...
				bi.setId(id-0*lastid); lastid = id; // TODO: FIX 0*
	...
	class RelationGroup extends Prim<Relation> implements PrimGroupWriterInterface {
		public void addStringsToStringtable() {
			StringTable stable = getStringTable();
	...
	/** Buffer up events into groups that are all of the same type, or all of the same length, then process each buffer */
	public class Processor implements EntityProcessor {
		@Override
		public void process(BoundContainer bound) {
			// Specialcase this. Assume we only ever get one contigious bound request.
			switchTypes();
	...
	/** At the end of this function, all of the lists of unprocessed 'things' must be null 
	 */
